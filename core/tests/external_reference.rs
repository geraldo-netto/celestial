...
fn equation_of_time_within_bounds() {
    ...
    let eot_apr = time_equ(JulianDay::new(jd_apr)).unwrap();
    ...
    for &doy in &[15.0_f64, 100.0, 200.0, 300.0] {
        ...
        let eot = time_equ(JulianDay::new(jd)).unwrap();
...
fn almuten_at_19_aries_returns_dignified_body() {
    let (body, score) = almuten(Longitude::new(19.0), true);
...
fn almuten_at_leo_includes_sun() {
    let (body, score) = almuten(Longitude::new(125.0), true);
    use celestial_core::body::Body;
    assert!(
...
fn egyptian_terms_aries_all_five() {
    ...
    assert_eq!(egyptian_terms_ruler(Longitude::new(8.0)), Body::VENUS);
    assert_eq!(egyptian_terms_ruler(Longitude::new(15.0)), Body::MERCURY);
    assert_eq!(egyptian_terms_ruler(Longitude::new(22.0)), Body::MARS);
    assert_eq!(egyptian_terms_ruler(Longitude::new(28.0)), Body::SATURN);
}
...
fn day_chart_classification() {
    ...
    let day = is_day_chart(Longitude::new(270.0), &c);
    let night = is_day_chart(Longitude::new(90.0), &c);
...
fn hindu_festivals_2024_count() {
    ...
    for f in &fests {
        let d = celestial_core::revjul(JulianDay::new(f.jd), Calendar::Gregorian);
...
fn next_first_quarter_after_new_moon() {
    ...
    let fq = next_first_quarter(JulianDay::new(nm + 0.5)).unwrap();
    let dt = fq - nm;
    assert!(
        (dt - 7.4).abs() < 1.5,
...
fn azalt_smoke_test() {
    ...
    let r = azalt(JulianDay::new(jd), 0, geopos, 1013.25, 15.0, xin);
...
fn coord_transform_round_trip() {
    ...
    let forward = coord_transform(input, Degrees::new(eps));
    let back = coord_transform(forward, Degrees::new(-eps));
    for i in 0..3 {
        assert!(
...
fn best_time_method_bracketed() {
    ...
    let jd = best_time_method(JulianDay::new(sunset), JulianDay::new(moonset));
    assert!(
...
fn vietnamese_month_start_within_synodic() {
    ...
    if let Some(start) = vietnamese_month_start_jd(JulianDay::new(jd)) {
        let dt = jd - start;
        assert!(
...
fn sun_cardinal_ingress_dates_2024() {
    ...
    for &(lon, y, m, d, h) in cases {
        let jd = solcross_ut(Longitude::new(lon), JulianDay::new(jd_start), FLG).unwrap();
...
fn ethiopic_round_trip_2017_ee() {
    ...
    let (y, m, d) = jd_to_ethiopic(JulianDay::new(jd));
    assert_eq!((y, m, d), (2017, 1, 1));
    let gd = celestial_core::revjul(JulianDay::new(jd), Calendar::Gregorian);
    assert!(
...
fn solar_eclipse_2024_april_08() {
    let jd_start = julday(2024, 1, 1, 0.0, Calendar::Gregorian);
    let r = sol_eclipse_when_glob(JulianDay::new(jd_start), FLG, 0, false);
    if let Ok(eclipse) = r {
...
fn calendar_round_period() {
    let jd0 = 2_451_545.0;
    let cr0 = calendar_round(JulianDay::new(jd0));
    let cr1 = calendar_round(JulianDay::new(jd0 + 18_980.0));
...
fn haab_3_kankin_at_2012_solstice() {
    let jd = julday(2012, 12, 22, 0.0, Calendar::Gregorian);
    let (month_idx, day, _name) = haab(JulianDay::new(jd));
    assert!(
        month_idx < 19,
...
fn parallactic_angle_canonical_geometries() {
    ...
    let q_west = parallactic_angle(Degrees::new(90.0), Degrees::new(0.0), Latitude::new(0.0));
    assert!(
...
fn parallactic_angle_symmetry() {
    use celestial_core::parallactic_angle;
    let q_pos = parallactic_angle(Degrees::new(30.0), Degrees::new(20.0), Latitude::new(40.0));
    let q_neg = parallactic_angle(Degrees::new(-30.0), Degrees::new(20.0), Latitude::new(40.0));
    assert!(
