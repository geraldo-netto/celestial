...
fn test_revjul_basic() {
    let d = revjul(JulianDay::new(2452275.5), Calendar::Gregorian);
...
#[test]
fn test_day_of_week() {
    assert_eq!(day_of_week(JulianDay::new(2452275.5)), 1); // 2002-01-01 = Tuesday (1)
    assert_eq!(day_of_week(JulianDay::new(2459444.0)), 1); // 2021-08-17 = Tuesday (1)
}

#[test]
fn test_deltat_j2000() {
    // ΔT at J2000.0 ≈ 63.8 s
    let dt = deltat(JulianDay::new(2451545.0));
    assert!(
...
fn test_deltat_reference() {
    ...
    let dt = deltat(JulianDay::new(2452275.5));
...
fn test_cotrans_poles() {
    ...
    let out = coord_transform([270.0, 90.0, 1.0], Degrees::new(23.4));
...
fn test_get_ayanamsa_fagan_bradley() {
    ...
    let ay = ayanamsa(JulianDay::new(2452275.5));
...
fn test_ayanamsa_lahiri_less_than_fagan() {
    ...
    set_sid_mode(SiderealMode::FAGAN_BRADLEY, 0.0, 0.0);
    let fb = ayanamsa(JulianDay::new(2451545.0));
    set_sid_mode(SiderealMode::LAHIRI, 0.0, 0.0);
    let la = ayanamsa(JulianDay::new(2451545.0));
...
fn test_sidtime_j2000() {
    setup();
    let st = sidtime(JulianDay::new(2451545.0));
...
fn test_sidtime_reference() {
    setup();
    let st = sidtime(JulianDay::new(2452275.5));
...
fn test_sol_eclipse_when_glob() {
    ...
    setup();
    let r = sol_eclipse_when_glob(JulianDay::new(2454466.5), CalcFlags::BUILTIN, 0, false).unwrap();
...
fn test_lun_eclipse_when() {
    ...
    setup();
    let r = lun_eclipse_when(JulianDay::new(2454466.5), CalcFlags::BUILTIN, 0, false).unwrap();
...
fn test_rise_trans_moon() {
    ...
    setup();
    let r = rise_trans(
        JulianDay::new(2459414.104_166_666_5),
        Body::MOON,
        None,
        CalcFlags::BUILTIN,
        CALC_RISE,
...
fn test_sidtime_reference_exact() {
    ...
    assert_approx!(sidtime(JulianDay::new(2452275.5)), 6.698_120_978_203_582);
}
...
fn test_solcross_equinox_2022() {
    ...
    let cross = solcross(Longitude::new(0.0), JulianDay::new(jd), CalcFlags::BUILTIN).unwrap();
    let d = revjul(JulianDay::new(cross), Calendar::Gregorian);
...
fn check_sign_ingress_baselines(j2000: f64, f: CalcFlags, tol: f64) {
    ...
    for (body, expected_jd, expected_sign) in cases {
        let (jd_ing, sign) = sign_ingress_ut(body, JulianDay::new(j2000), f, false).unwrap();
...
fn precision_search_root_finder_baselines() {
    ...
    assert!(
        lon_err < 1.0e-6,
        "Sun lon err at vernal eq = {lon_err:.2e}°"
    );
}
...
fn test_set_delta_t_userdef_overrides() {
    ...
    let dt_auto = deltat(JulianDay::new(2_451_545.0));
...
fn precision_nutation_meeus_22a() {
    ...
    let jde = 2_446_895.5;
    let (nut_lon, nut_obl) = nutation(JulianDay::new(jde));
...
fn precision_obliquity_meeus_22b() {
    ...
    let jde = 2_446_895.5;
    let eps = true_obliquity(JulianDay::new(jde));
    assert!(
...
fn precision_sidtime_meeus_12a() {
    ...
    let jd = 2_446_895.5;
    let gst = sidtime(JulianDay::new(jd));
    assert!(
...
fn precision_ayanamsa_lahiri_j2000() {
    ...
    set_sid_mode(SiderealMode::LAHIRI, 0.0, 0.0);
    let ay = ayanamsa(JulianDay::new(2_451_545.0));
    assert!(
        (ay - 23.855).abs() < 0.05,
...
fn precision_time_equ_reference() {
    ...
    let e = time_equ(JulianDay::new(jd)).unwrap();
...
fn solar_return_jd_consecutive_years_one_year_apart() {
    let sr2024 = solar_return_jd(JulianDay::new(JD_NATAL), 2024, CalcFlags::BUILTIN).unwrap();
    let sr2025 = solar_return_jd(JulianDay::new(JD_NATAL), 2025, CalcFlags::BUILTIN).unwrap();
...
fn full_dignity_real_chart_j2000() {
    ...
    .unwrap();
    let is_day = is_day_chart(Longitude::new(sun.lon), &h.cusps);
    let (dig, score) = full_dignity(Body::SUN, Longitude::new(sun.lon), is_day);
...
fn firdaria_real_chart_covers_75_years() {
    ...
    let is_day = is_day_chart(Longitude::new(sun.lon), &h.cusps);
    let periods = firdaria(JulianDay::new(JD_NATAL), is_day, 75.0);
...
fn four_pillars_j2000_known_values() {
    let sun = calc_ut(JulianDay::new(JD_J2000), Body::SUN, CalcFlags::BUILTIN).unwrap();
    let pillars = four_pillars(JulianDay::new(JD_J2000), 12.0, Longitude::new(sun.lon));
...
fn four_pillars_hour_pillar_changes_every_2_hours() {
    ...
    let p0 = four_pillars(JulianDay::new(JD_J2000), 0.0, Longitude::new(sun.lon));
    let p1 = four_pillars(JulianDay::new(JD_J2000), 1.0, Longitude::new(sun.lon));
    let p3 = four_pillars(JulianDay::new(JD_J2000), 3.0, Longitude::new(sun.lon));
...
fn calendar_round_repeats_after_18980_days() {
    let cr0 = calendar_round(JulianDay::new(JD_J2000));
    let cr1 = calendar_round(JulianDay::new(JD_J2000 + 18_980.0));
...
fn egyptian_decan_j2000_sun() {
    let sun = calc_ut(JulianDay::new(JD_J2000), Body::SUN, CalcFlags::BUILTIN).unwrap();
    let (idx, name, star) = egyptian_decan(Longitude::new(sun.lon));
