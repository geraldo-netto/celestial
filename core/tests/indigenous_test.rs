...
mod indigenous {
    ...
    fn medicine_wheel_covers_all_360() {
        ...
        for i in 0..360u32 {
            ...
            let (animal, _, _, _) = medicine_wheel_totem(Longitude::new(lon));
            assert!(
    ...
    fn medicine_wheel_returns_four_elements() {
        ...
        for i in 0..360u32 {
            let (_, element, _, _) = medicine_wheel_totem(Longitude::new(i as f64 + 0.5));
    ...
    fn egyptian_decan_covers_all_36() {
        ...
        for i in 0..36u32 {
            ...
            let (idx, _, _) = egyptian_decan(Longitude::new(lon));
            seen.insert(idx);
        }
    ...
    fn egyptian_decan_idx_in_range() {
        for i in 0..360u32 {
            let (idx, name, star) = egyptian_decan(Longitude::new(i as f64 + 0.5));
...
mod extra_indigenous {
    ...
    fn medicine_wheel_returns_valid_season() {
        ...
        for i in 0..36 {
            ...
            let (_, _, _, season) = medicine_wheel_totem(Longitude::new(lon));
            assert!(
    ...
    fn medicine_wheel_returns_valid_element() {
        ...
        for i in 0..36 {
            let lon = i as f64 * 10.0;
            let (_, element, _, _) = medicine_wheel_totem(Longitude::new(lon));
            assert!(
    ...
    fn medicine_wheel_returns_valid_clan() {
        ...
        for i in 0..36 {
            let lon = i as f64 * 10.0;
            let (_, _, clan, _) = medicine_wheel_totem(Longitude::new(lon));
    ...
    fn medicine_wheel_wraps_at_360() {
        let (a0, e0, c0, s0) = medicine_wheel_totem(Longitude::new(0.0));
        let (a360, e360, c360, s360) = medicine_wheel_totem(Longitude::new(360.0));
        assert_eq!(a0, a360);
        assert_eq!(e0, e360);
        assert_eq!(c0, c360);
        assert_eq!(s0, s360);
    ...
    fn egyptian_decan_wraps_at_360() {
        let (i0, n0, s0) = egyptian_decan(Longitude::new(0.0));
        let (i360, n360, s360) = egyptian_decan(Longitude::new(360.0));
        assert_eq!(i0, i360);
        assert_eq!(n0, n360);
        assert_eq!(s0, s360);
    }
    ...
    fn egyptian_decan_names_unique() {
        let names: Vec<_> = (0..36)
            .map(|i| egyptian_decan(Longitude::new(i as f64 * 10.0 + 1.0)).1)
            .collect();
