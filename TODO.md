...
| BIND-4 | OPEN | L | 114 core flat public fns are exposed by no binding (`xtask/core_unbound_allow.txt`). Most are intentionally Rust-only (low-level `centisec_*`/`deg_to_cs`, path/config setters, unwrapped library APIs — see WIRE-1), but some are plausibly bindable (`arabic_parts_seven`, `sign_ruler`, `hebrew_month_days`, `next_full_moon_after`). | The allow-list is now the visible, gated backlog: `cargo xtask coverage` forces every FUTURE core fn to be bound-or-listed, so the gap can only shrink. Chip away by binding the bindable ones in batches and deleting their allow-list lines. Superseded by GATE-3/GATE-4. |
| GATE-2 | OPEN | S | Arity check (B) counts params but not ORDER or TYPE. `lun_eclipse_how` is `(jd_ut, geopos, flags)` in Python vs `(jd_ut, flags, geopos)` in JS — silent arg-swap. | Upgrade the coverage arity check to compare the ordered `(type, name)` signature per fn across bindings, reusing `parse_fn_sig`. |
| GATE-3 | OPEN | S | `core_unbound_allow.txt` doesn't distinguish "Rust-only forever" from "not-yet-bound backlog", so the real gap size is uncountable. | Annotate each entry `# rust-only` vs `# backlog` (WIRE-1 items = rust-only). Makes GATE-4 scope explicit. |
| GATE-4 | OPEN | L | ~40 genuinely useful core fns unbound: lunar occultations (`lun_occult_when_glob/loc/where`), planetary phenomena (`pheno`, `pheno_ut`), `heliacal_ut`, `nod_aps`, calendar detail (`hebrew_month_days`, `hijri_new_year_jd`, `islamic_observances_for_jd`), etc. | Bind in batches across Python+JS+PHP, regenerate stubs/shapes/apidoc, delete their allow-list lines. Depends on GATE-3 triage. |
| GATE-5 | OPEN | M | No cross-language RUNTIME behaviour parity: nothing calls the built addons and diffs `calc_ut`/`houses` numbers across Python/JS/PHP/core. `reference_values.json` only covers ~15 categories in pure-logic (re-implemented math, not the native call). | Add a golden harness in the build CI jobs that loads each built binding and asserts identical numeric output vs a core-generated fixture. |
