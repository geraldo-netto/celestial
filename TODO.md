...
| MUT-6 | OPEN | M | Nine `crossings` predicate mutants still time out instead of failing fast: five in `opposite_sign`, three in `brackets_root`, and `&&` to `||` in `bracket_crossing`. Re-measured after REL-44 replaced the product tests, so the two mutants named in the original row (`*` to `+`, `<=` to `>`) no longer exist, but the timeout class survives the rewrite. | The hang is not slow tests: re-running the same mutants with `--timeout 300` still times out `replace opposite_sign -> bool with true`, so a broken sign predicate drives the crossing search into an effectively unbounded loop. Direct unit assertions on the predicates already exist and are not enough — the fix belongs in production code, bounding `find_crossing` / `refine_crossing` by wall-clock or probe count so a false bracket fails instead of spinning. |
