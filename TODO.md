...
Rescan: **2026-07-02c** (follow-up all-category rescan incl. the core-correctness cluster that aborted in 02b; 4 parallel audit agents — core astronomy math numerically verified with a scratch harness, core infra/concurrency, soft categories verified by executing the shipped binary, cli-render/xtask re-sweep). New: REL-16..28, CONC-1/2, SCALE-1, ROB-3, PROD-1, DOC-13, CLI-4, UX-2, WIRE-4 (+REL-12 note: two more double-conversion sites). Headline: Vedic render uses `CalcFlags(64)`=FLG_NONUT believing it's FLG_SIDEREAL → all Vedic charts tropical (REL-22); eclipse gamma formulas non-Meeus → phantom eclipses + everything classified TOTAL (REL-16/17); `calc_many` worker threads silently drop `set_sid_mode`/`set_topo` (CONC-1). Checked-clean: ELP2000 vs Meeus 47.a, computus, molad/dechiyot, tabular Hijri, coptic/ethiopic, no static-mut/atomics in core, SVG goldens pinned.
...
| CONC-1 | OPEN | M | `calc_many`/`calc_ut_many` (`core/src/functions/calc.rs:299`) spawn scoped OS threads whose thread-local `CONFIG` re-initializes to defaults, so workers silently ignore the caller's `set_sid_mode` (engine.rs:178), `set_topo` (engine.rs:190), and `set_delta_t_userdef`. The ≤2-body sequential fast-path (calc.rs:289) DOES see the caller's config — same FLG_SIDEREAL/FLG_TOPOCTR call returns different longitudes depending on body count. Exposed verbatim through Python/JS/PHP `calc_ut_many`. | Capture `cfg()` in the caller and seed each worker's thread-local (or pass an explicit `EngineConfig` into `calc_tt`) before invoking `f(body)`. |
...
| REL-12 | OPEN | S | `core/src/astronomy/eclipses.rs:191` computes the Besselian `u` half-width term as `0.0004 * to_rad(2.0 * to_rad(mp).sin()).cos()` — it wraps `mp` through `to_rad().sin()` before the outer `to_rad().cos()`, so it is not the Meeus 54.x term `0.0004 * cos(2·M')`. | Small coefficient (0.0004 Earth-radii) so eclipse-type classification rarely flips, but the value is mathematically wrong. Should be `0.0004 * to_rad(2.0 * mp).cos()`. Add a reference-eclipse regression when fixing. 2026-07-02c: two MORE sites of the same double-conversion class found — `solar_eclipse_magnitude` (eclipses.rs:153, `0.0004*to_rad(2.0*mp.to_radians()).cos()`) and lunar sigma (eclipses.rs:236, `0.0105*…`); fold all three into one fix pass with REL-16/17. |
...
| REL-22 | OPEN | M | All four "sidereal" CLI render sites pass `CalcFlags(64)` with a `// sidereal` comment (`cli/src/cmd/render/vedic.rs:159,417,697`, `cli/src/cmd/render/mod.rs:1287`), but 64 = `FLG_NONUT` (constants.rs:168); `FLG_SIDEREAL` = 65536 (constants.rs:195) and the engine only subtracts ayanamsa on that bit (engine.rs:68). No ayanamsa is applied anywhere in the vedic path (grep: zero `ayanamsa`/`set_sid_mode` hits) — every Vedic chart's rasi/navamsa/nakshatra/dasha is computed from TROPICAL longitudes, ~24° off; most bodies land in the wrong rasi. README:502 promises Lahiri sidereal. | Use `CalcFlags(celestial_core::FLG_SIDEREAL)` + `set_sid_mode(SIDM_LAHIRI,0,0)` at all four sites; re-pin the vedic SVG goldens (current goldens pin the buggy tropical output). Same DOC-9 "64" folklore that hit the docs. |
| REL-16 | OPEN | M | `check_solar_eclipse` gamma (`core/src/astronomy/eclipses.rs:180-187`) uses `1.0128*sin_f` then halves it (`gamma = 0.5*gamma  // approximate centring`) instead of Meeus 54.x `P·cos F1 + Q·sin F1` (Q≈5.22), so γ maxes ~0.3 and the `>0.9972` partial/no-eclipse branches are unreachable. Numerically verified: phantom eclipses reported 2025-02-28 and 2025-08-23 (none exist); real partials 2025-03-29/2025-09-21 classified TOTAL\|CENTRAL / ANNULAR\|CENTRAL. | Implement the Meeus P/Q/W gamma so partial classification and no-eclipse rejection fire; add reference-eclipse regressions. |
| REL-17 | OPEN | M | `check_lunar_eclipse` (`core/src/astronomy/eclipses.rs:240-246`) uses `gamma_abs = sin_f.abs()` (≤0.36, never scaled by ~5.22) so every full moon within ~21° of the node classifies `TotalLunar`; sigma is computed then discarded via `sig * 0.0` (:245). Verified: non-eclipses 2023-09-29/2024-04-23 reported as eclipses; penumbral 2023-05-05/2024-03-25 and partial 2023-10-28/2024-09-18 all returned TOTAL — Partial/Penumbral kinds unreachable. | Compute the real gamma (P·cosF1+Q·sinF1) and use Meeus pen/umb magnitude formulas. |
| REL-18 | OPEN | M | `rise_set.rs:165` feeds normalized 0–360° RA straight into quadratic `interpolate(ra0,ra1,ra2,m)` with no wrap unwrap, so once a month when the Moon's RA crosses 360°→0° rise/set times are hours wrong. Verified lat 40/lon 0 Jan 2024: day-to-day moonrise deltas hit 20.5h/27.2h (Jan 16 rise 3.5h early) vs smooth ~24.5-25.2h. | Unwrap ra0/ra1/ra2 into a monotonic sequence (+360 where consecutive values decrease) before interpolating. |
| REL-19 | OPEN | S | `sol_eclipse_how` (`core/src/functions/eclipses.rs:105`) sets `ret_flags` from `attr[1] > 0.0`, but `attr[1]` is the constant solar diameter 0.5266 — so it returns `ECL_TOTAL` for every input date (verified on 3 arbitrary non-eclipse dates; magnitude attr[0]≈2.0 impossible, from the broken pen/umb formulas at astronomy/eclipses.rs:146-157). | Key classification off `check_solar_eclipse`'s kind; fix magnitude to Meeus 54.2 `(1.5433+u−\|γ\|)/(0.5461+2u)`. Depends on REL-16. |
