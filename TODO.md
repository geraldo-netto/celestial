...
| BIND-4 | OPEN | L | 114 core flat public fns are exposed by no binding (`xtask/core_unbound_allow.txt`). Most are intentionally Rust-only (low-level `centisec_*`/`deg_to_cs`, path/config setters, unwrapped library APIs — see WIRE-1), but some are plausibly bindable (`arabic_parts_seven`, `sign_ruler`, `hebrew_month_days`, `next_full_moon_after`). | The allow-list is now the visible, gated backlog: `cargo xtask coverage` forces every FUTURE core fn to be bound-or-listed, so the gap can only shrink. Chip away by binding the bindable ones in batches and deleting their allow-list lines. Now split `# rust-only` (29) vs `# backlog` (85) in the file; see GATE-4. |
| GATE-4 | OPEN | L | 74 core fns remain `# backlog` in `xtask/core_unbound_allow.txt` (was 85; batch 1 bound the 11 Hebrew/Hijri/Bahá'í calendar-detail helpers). Remaining useful clusters: lunar occultations (`lun_occult_when_glob/loc/where`), planetary phenomena (`pheno`, `pheno_ut`), `heliacal_ut`, `nod_aps`, more calendar detail (`islamic_observances_for_jd`, `hebrew_year_from_jd`). | Continue binding in batches across Python+JS+PHP, regenerate stubs/shapes/apidoc + `coverage --write-allow`. `cargo xtask coverage` prints the shrinking backlog count. Refine mis-tagged `backlog`→`rust-only` in `classify_unbound` as encountered. |
