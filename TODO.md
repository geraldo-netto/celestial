...
| GATE-4 | OPEN | L | 85 core fns are `# backlog` (bindable, not yet wrapped) in `xtask/core_unbound_allow.txt`; 29 are `# rust-only`. Useful clusters: lunar occultations (`lun_occult_when_glob/loc/where`), planetary phenomena (`pheno`, `pheno_ut`), `heliacal_ut`, `nod_aps`, calendar detail (`hebrew_month_days`, `hijri_new_year_jd`, `islamic_observances_for_jd`). | Bind in batches across Python+JS+PHP, regenerate stubs/shapes/apidoc, delete their allow-list lines. `cargo xtask coverage` prints the shrinking backlog count. Refine any mis-tagged `backlog`→`rust-only` in `classify_unbound`. |
