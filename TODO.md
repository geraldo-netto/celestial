...
| COV-1 | DECIDED | S | llvm-cov `--fail-under-functions 90` was failing at 86.28%. `--fail-under-lines 80` passing at 85.35%. Root cause: llvm-cov counts mangled-name fn *instantiations*; each test/binary that links `celestial-core` produces its own monomorphization, and fns not called from that binary stay at count 0 there. After dedup-by-source-location only **18** of ~1500 distinct fns were truly uncov. Compounded by stale profraw from prior CI builds inflating the missed-fn count. | Fix 2026-05-27: (a) added `#[cfg(test)] mod cov_tests` blocks for the 18 reachable internal helpers; (b) deleted dead pub fns `eclipses::saros`, `crossings::{helio_cross,mooncross_node}_ut` (orphan — not re-exported from `lib.rs`, zero non-test callers); (c) added direct `do_exec` test via `ErrorKind::NotFound` path (process-replacement path remains untestable in-process by design); (d) prepended `cargo llvm-cov clean --workspace` to CI step so each run starts with empty profraw; (e) raised gate to `--fail-under-{lines,functions} 93`. Result: lines 95.43%, fns 95.65%, only `aspects::match_core::<_>` and `do_exec` keep us under 100% — both have unavoidable monomorphization noise. |
