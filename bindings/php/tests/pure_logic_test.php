...
function assertApprox(float $actual, float $expected, float $tol = 1e-6, string $label = ''): void {
    global $passed, $failed;
    if (abs($actual - $expected) <= $tol) {
        $passed++;
    } else {
        $failed++;
        $name = $label ?: 'assertApprox';
        echo "FAIL [$name]: expected $expected, got $actual (diff " . abs($actual - $expected) . ")\n";
    }
}

function assertEq(mixed $actual, mixed $expected, string $label = ''): void {
    global $passed, $failed;
    if ($actual === $expected) {
        $passed++;
    } else {
...
assertEq(SUN,  0, 'SUN constant');
assertEq(MOON, 1, 'MOON constant');
assertEq(GREG_CAL, 1, 'GREG_CAL constant');

// ── Time & calendar ────────────────────────────────────────────────────────────

// julday / revjul round-trip
$jd = julday(2002, 1, 1, 0.0, GREG_CAL);
assertApprox($jd, 2452275.5, 1e-6, 'julday 2002-01-01');

$d = revjul($jd, GREG_CAL);
assertEq((int)$d['year'],  2002, 'revjul year');
assertEq((int)$d['month'],    1, 'revjul month');
assertEq((int)$d['day'],      1, 'revjul day');
assertApprox($d['hour'], 0.0, 1e-6, 'revjul hour');

// Day of week: 2452275.5 = Tuesday (1)
assertEq(day_of_week($jd), 1, 'day_of_week');

// Delta T at J2000 ≈ 63.8 s ≈ 0.000738 days
$dt = deltat(2451545.0);
assertApprox($dt, 0.000738, 0.001, 'deltat J2000');

// ── Planetary positions ─────────────────────────────────────────────────────────

$sun = calc_ut($jd, SUN, FLG_BUILTIN | FLG_SPEED);
assertEq(count($sun), 6, 'calc_ut returns 6 elements');

$lon = $sun[0];
assertApprox($lon, 280.38, 0.1, 'Sun longitude 2002-01-01');  // ~280°

$dist = $sun[2];
// Earth–Sun distance in January ≈ 0.9832 AU
assertApprox($dist, 0.9832, 0.005, 'Sun distance Jan');

// Speed: Sun moves ~1°/day
$speed = $sun[3];
assertApprox($speed, 1.0, 0.1, 'Sun speed');

// Moon longitude is in [0, 360)
$moon = calc_ut($jd, MOON, FLG_BUILTIN);
assertApprox($moon[0] >= 0.0 && $moon[0] < 360.0 ? 1.0 : 0.0, 1.0, 0, 'Moon lon range');

// ── Houses ──────────────────────────────────────────────────────────────────────

$h = houses($jd, 48.85, 2.35, ord('P'));  // Paris, Placidus
assertEq(count($h['cusps']),  12, 'houses cusps count');
assertEq(count($h['ascmc']),  8, 'houses ascmc count');

$asc = $h['ascmc'][0];
assertApprox($asc >= 0.0 && $asc < 360.0 ? 1.0 : 0.0, 1.0, 0, 'ASC in [0,360)');

$h_ex = houses_ex($jd, 48.85, 2.35, ord('P'), FLG_BUILTIN);
assertEq(count($h_ex['cusps']), 12, 'houses_ex cusps count');
assertEq(count($h_ex['ascmc']), 8, 'houses_ex ascmc count');

// house_name
assertEq(house_name(ord('P')), 'Placidus', 'house_name Placidus');
assertEq(house_name(ord('K')), 'Koch',     'house_name Koch');
...
assertApprox($ay, 23.88, 0.1, 'Lahiri ayanamsa');

assertEq(ayanamsa_name(SIDM_LAHIRI), 'Lahiri', 'ayanamsa name');
...
assertEq((int)$eq_date['month'], 3, 'Ostara in March');
assertEq((int)$eq_date['year'],  2002, 'Ostara year 2002');

// ── Coordinate transforms ────────────────────────────────────────────────────────

$norm = degnorm(361.5);
assertApprox($norm, 1.5, 1e-10, 'degnorm(361.5)');

$norm2 = degnorm(-1.0);
assertApprox($norm2, 359.0, 1e-10, 'degnorm(-1)');

// difdeg2n(p1,p2) = p1-p2 normalized to (-180,+180]: 10-350=-340 → +20
$diff = difdeg2n(10.0, 350.0);
assertApprox($diff, 20.0, 1e-10, 'difdeg2n(10,350)=+20');

$parts = split_deg(123.456, 0);
assertEq(count($parts), 5, 'split_deg count');
assertEq((int)$parts[0], 123, 'split_deg degrees');

// ── Celtic sabbats ───────────────────────────────────────────────────────────────

$ostara_jd = sabbat_jd(2025, 'Ostara');
$ostara = revjul($ostara_jd, GREG_CAL);
assertEq((int)$ostara['month'], 3, 'Ostara 2025 in March');
assertEq((int)$ostara['year'],  2025, 'Ostara 2025 year');

$yule_jd = sabbat_jd(2025, 'Yule');
$yule = revjul($yule_jd, GREG_CAL);
assertEq((int)$yule['month'], 12, 'Yule 2025 in December');

$all_sabbats = sabbats_for_year(2025);
assertEq(count($all_sabbats), 8, 'sabbats_for_year returns 8');

// Next sabbat from Jan 1, 2025 should be Imbolc (Feb 1)
$jd_jan = julday(2025, 1, 15, 0.0, GREG_CAL);
$next_name = next_sabbat_name($jd_jan);
assertEq($next_name, 'Imbolc', 'next sabbat from Jan 15');
...
// Jan 25, 2024
assertEq((int)$fm_date['month'], 1, 'First FM 2024 in January');

$esbats_2024 = esbats_for_year(2024);
assertApprox(count($esbats_2024) >= 12 ? 1.0 : 0.0, 1.0, 0, 'esbats_for_year >= 12');

// ── Vedic helpers ────────────────────────────────────────────────────────────────

$rasi = long_to_rasi(45.0);  // 45° = Taurus (1)
assertEq($rasi, 1, 'long_to_rasi 45° = Taurus');

$nav = long_to_navamsa(0.0);
assertEq($nav >= 0 && $nav < 12 ? 1 : 0, 1, 'navamsa in [0,12)');

$nak = long_to_nakshatra(0.0);
assertEq(count($nak), 2, 'nakshatra returns [nak, pada]');
assertApprox($nak[0] >= 0 && $nak[0] < 27 ? 1.0 : 0.0, 1.0, 0, 'nakshatra in [0,27)');
...
$name = sign_name(0);
assertEq($name !== null ? 1 : 0, 1, 'sign_name(0) not null');

// ── Format / parse coord ────────────────────────────────────────────────────────

$formatted = format_coord(51.5, true);
assertEq($formatted !== null ? 1 : 0, 1, 'format_coord not null');

$parsed = parse_coord('51N30');
assertApprox($parsed ?? 0.0, 51.5, 0.1, 'parse_coord 51N30');
...
assertApprox($mid, 15.0, 0.001, 'midpoint(10, 20) = 15');

$mid2 = midpoint(350.0, 10.0);
// Shorter arc midpoint wraps: 0° or 180°
assertEq($mid2 >= 0.0 && $mid2 < 360.0 ? 1 : 0, 1, 'midpoint in [0,360)');

// arabic_part: ASC + Moon - Sun
$fortune = arabic_part(206.77, 223.32, 280.38);
assertApprox($fortune, 149.71, 0.1, 'arabic_part lot of fortune');
assertEq($fortune >= 0.0 && $fortune < 360.0 ? 1 : 0, 1, 'arabic_part in range');
...
assertEq($aries_ruler,  4, 'Aries ruler = Mars');
assertEq($leo_ruler,    0, 'Leo ruler = Sun');
assertEq($cancer_ruler, 1, 'Cancer ruler = Moon');

// zodiac_sign_name
$name = zodiac_sign_name(0);
assertEq($name === 'Aries' ? 1 : 0, 1, 'zodiac_sign_name(0) = Aries');
$name11 = zodiac_sign_name(11);
assertEq($name11 === 'Pisces' ? 1 : 0, 1, 'zodiac_sign_name(11) = Pisces');

// lon_to_sign: longitude → [sign, degrees]
$result = lon_to_sign(45.5);   // Taurus 15.5°
assertApprox($result[0], 1.0, 0.001, 'lon_to_sign(45.5) sign = 1 (Taurus)');
assertApprox($result[1], 15.5, 0.001, 'lon_to_sign(45.5) deg = 15.5');

$r0 = lon_to_sign(0.0);
assertApprox($r0[0], 0.0, 0.001, 'lon_to_sign(0) = Aries');

// degnorm
$n1 = degnorm(360.0);
assertApprox($n1, 0.0, 1e-9, 'degnorm(360) = 0');
$n2 = degnorm(-1.0);
assertApprox($n2, 359.0, 1e-9, 'degnorm(-1) = 359');
$n3 = degnorm(720.5);
assertApprox($n3, 0.5, 1e-9, 'degnorm(720.5) = 0.5');

// difdeg2n
$d = difdeg2n(360.5, 540.0);
assertApprox($d, -179.5, 1e-9, 'difdeg2n(360.5, 540) = -179.5');

// version
$v = version();
assertEq(strlen($v) > 0 ? 1 : 0, 1, 'version() not empty');
...
assertApprox($gmst_hours, 18.697374491, 0.001, 'GMST at J2000 = 18.69737449 h');
...
assertEq($eq_eq_h > 0 ? 1 : 0, 1, 'equation of equinoxes > 0');
assertEq($eq_eq_h < (1.0 / 3600.0) ? 1 : 0, 1, 'equation of equinoxes < 1s');

// GMST advances ~360° per sidereal day
$advance = 360.98564736629 * 0.99726958;
assertEq(abs($advance - 360.0) < 1.0 ? 1 : 0, 1, 'GMST advances ~360° per sidereal day');
...
assertEq($expected_shift > 0.005 ? 1 : 0, 1, 'calc(TT) vs calc_ut delta > 0.005 deg');
assertEq($expected_shift < 1.0 ? 1 : 0, 1, 'calc(TT) vs calc_ut delta < 1 deg');

// Meeus §47.a: Moon TT reference 133.167° is in Leo (120-150°)
$moon_ref = 133.167;
assertEq(($moon_ref >= 120.0 && $moon_ref < 150.0) ? 1 : 0, 1,
    'Meeus Moon TT ref 133.167° is in Leo');

// Meeus §25.a: Sun TT reference 199.909° is in Libra (180-210°)
$sun_ref = 199.909;
assertEq(($sun_ref >= 180.0 && $sun_ref < 210.0) ? 1 : 0, 1,
    'Meeus Sun TT ref 199.909° is in Libra');

// delta-T polynomial at J2000: ΔT = 63.87 + 0.3345*T + 0.0094*T² (T=0)
$dt_j2000 = 63.87;
assertEq(abs($dt_j2000 - 63.83) < 1.0 ? 1 : 0, 1,
...
assertApprox($eps0_j2000, 23.439291, 0.0001,
...
assertApprox($eps0_1987, 23.44094, 0.001,
...
assertEq(abs($contrib_arcsec) > 3.0 && abs($contrib_arcsec) < 4.0 ? 1 : 0, 1,
...
$a = (180.0 - 15.0) % 360.0;
assertApprox($a, 165.0, 1e-9, '15° Aries antiscion = 15° Virgo (165°)');

$a = (180.0 - 90.0) % 360.0;
assertApprox($a, 90.0, 1e-9, '0°Cancer antiscion = itself');

$a = (180.0 - 270.0 + 360.0) % 360.0;
assertApprox($a, 270.0, 1e-9, '0°Capricorn antiscion = itself');
...
foreach ([0.0, 45.0, 90.0, 135.0, 180.0, 225.0, 270.0, 315.0] as $lon) {
    ...
    assertApprox($a2, $lon, 1e-9, "double antiscion of $lon");
}

// ── Phase 1: minor aspects pure-logic ────────────────────────────────────────

assertApprox(360.0 / 5.0, 72.0,  1e-9, 'quintile = 72°');
assertApprox(360.0 / 9.0, 40.0,  1e-9, 'novile = 40°');
assertApprox(360.0 / 8.0, 45.0,  1e-9, 'semi-square = 45°');
assertApprox(3 * 360.0 / 8.0, 135.0, 1e-9, 'sesquiquadrate = 135°');
assertApprox(360.0 / 7.0, 51.4286, 0.001, 'septile ≈ 51.43°');

// ── Phase 1: arabic parts formula check ──────────────────────────────────────

$asc = 0.0; $sun = 30.0; $moon = 120.0;
$fortune_day = fmod($asc + $moon - $sun + 360.0, 360.0);
assertApprox($fortune_day, 90.0, 1e-9, 'Lot of Fortune day = ASC+Moon-Sun');
$fortune_night = fmod($asc + $sun - $moon + 360.0, 360.0);
assertApprox($fortune_night, 270.0, 1e-9, 'Lot of Fortune night = ASC+Sun-Moon');
...
assertEq(count($pos),       6,   'DEAD-3 pos6: array length is 6');
assertEq(is_array($pos)?1:0, 1,  'DEAD-3 pos6: result is an array');
for ($i = 0; $i < 6; $i++) {
    assertEq(is_float($pos[$i]) ? 1 : 0, 1, "DEAD-3 pos6: slot $i is float");
    assertEq(is_finite($pos[$i])  ? 1 : 0, 1, "DEAD-3 pos6: slot $i is finite");
}
...
assertEq($pos[2] > 0.95 && $pos[2] < 1.05 ? 1 : 0, 1, 'DEAD-3 pos6: slot 2 ≈ 1 AU (Sun distance)');
assertEq($pos[3] > 0.0  && $pos[3] < 2.0  ? 1 : 0, 1, 'DEAD-3 pos6: slot 3 ≈ Sun speed_lon');
...
foreach ([21, 39, 5000, -2, -11, 1_010_000] as $bad) {
    ...
    assertEq($threw ? 1 : 0, 1, "REL-8: invalid body id $bad must throw");
}
...
foreach ([SUN, MOON, MERCURY, VENUS, MARS, JUPITER, SATURN, URANUS, NEPTUNE, PLUTO] as $good) {
    ...
    assertEq($ok ? 1 : 0, 1, "REL-8: documented body id $good must succeed");
}
